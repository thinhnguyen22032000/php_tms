        ...
        <div class="card">
            <div class="form-group row ml-2">
                <div class="col col-sm-10 col-lg-9 offset-sm-1 offset-lg-0 mt-2">
                    <button type="submit" id="btn-submit" class="btn btn-space btn-primary">Edit</button>
                    <a href="<?= $GLOBALS['url'] ?>/request" class="btn btn-space btn-secondary">Cancel</a>
                </div>
            </div>
            <!-- <h5 class="card-header">Basic Form</h5> -->
            <form id="form" class="card-body" method="post" action="<?= $GLOBALS['url'] ?>/request/update/<?=$reqData['id']?>">
                    ...
                    <div class="col-md-6 col-sm-12 col-lg-6">
                        <div class="form-group">
                            ...
                            <select class="form-select form-control" name="partial_day" id="parial-day">
                                <option <?=$reqData['partial_day'] == 'all'?'selected':''?> value="all">All Day</option>
                                <option <?=$reqData['partial_day'] == 'mor'?'selected':''?> value="mor">Morning</option>
                                <option <?=$reqData['partial_day'] == 'aft'?'selected':''?> value="aft">Afternoon </option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-lg-6">
                ...
                <div class="row">
                    <div class="col-md-6 col-sm-12 col-lg-6">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="detail_reason" id="detail-reason" rows="3"><?=$reqData['detail_reason']?></textarea>
                            <div class="invalid-feedback" id="detail-reason-msg">
                                Detail reason is require.
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-lg-6">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Remainning Quotas</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= array_key_exists('num_day_off', $time_data)?$time_data['num_day_off'] + $reqData['duration']: 0 ?></td>
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
        const app = {
            ...
            handleDate: {
                ...
                init() {
                    let start = this.untils.fomatDate('<?= $reqData['start_date'] ?>');
                    let end = this.untils.fomatDate('<?= $reqData['end_date'] ?>');
                    let expected = this.untils.fomatDate('<?= $reqData['expected_date'] ?>');
                    $('#start-date').val(start) //'2022-10-09'
                    $('#end-date').val(end) //'2022-10-09'
                    $('#expected-date').val(expected)
                    if('<?= $reqData['partial_day'] ?>' != 'all'){
                         app.duration = '<?= $reqData['duration'] * 2 ?>'
                         $('#duration').val(app.duration / 2 + ' day');
                    }else{
                        app.duration = '<?= $reqData['duration']?>'
            ...
            handleSelect: {
                ...
                init(){
                   this.untils.handlePolicy('<?=$reqData['request_type_id']?>')
                   this.untils.handleRequireDetailReason('<?=$reqData['reason_id']?>')
                ...
                listen() {
                    ...
                    $('#request-type').change(function(e) {
                        ...
                        _this.untils.handlePolicy(reType);
            ...
            api: {
                    ...
                    fetch('http://localhost/mini_project_2/requesttype/request_type')
                        ...
                        .then((data) => {
                            for (let i = 0; i < data.length; i++) {
                                if(data[i].id == '<?=$reqData['request_type_id']?>'){
                                    $('#request-type').append(`<option selected value="${data[i].id}">${data[i].name}</option>`)
                                }else{
                ...
                getReasons(requestType = '<?=$reqData['request_type_id']?>') {
                    fetch(`http://localhost/mini_project_2/reason/reasons_by_id/${requestType}`)
                        .then((response) => response.json())
                        .then((data) => {
                            $('#reasons').empty();
                            for (let i = 0; i < data.length; i++) {
                                if(data[i].id == '<?=$reqData['reason_id']?>'){
                                    $('#reasons').append(`<option selected value="${data[i].id}">${data[i].content}</option>`);
                                }else{
                                    $('#reasons').append(`<option value="${data[i].id}">${data[i].content}</option>`)
                                }
                            }
                            if('<?=$reqData['reason_id']?>' == ''){
                                console.log('isnull')
                                $('#reasons').append(`<option selected value="">Other</option>`);
                            }else{
                                $('#reasons').append(`<option value="">Other</option>`);
                                console.log('not isnull')
                            }
                        });
                },
                // getTimeOff() {
                ...
                //             app.num_day_off = data.num_day_off - '<?=$reqData['duration']?>'
                //             console.log(data)
                //         });
                // },
            },
